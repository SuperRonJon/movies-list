...
<div class="w-1/3 mx-auto mb-10">
    <TextInput on:keyup={(event) => handleKeyUp(event)} bind:value={inputValue} placeholder="Enter name of new Collection then press Enter..." />
</div>

{#each data.collections as collection}
    <div class="text-center">
        <a class="text-blue-600 hover:underline text-center" href="/collections/{collection.id}">{collection.name}</a>
    </div>
{/each}